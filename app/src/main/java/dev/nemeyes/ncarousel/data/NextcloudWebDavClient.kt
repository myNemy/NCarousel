 ...
 *
 * Authenticated files live under `remote.php/dav/files/{username}/...` with Basic Auth.
 */
class NextcloudWebDavClient(
    ...
    /**
     * Lists image file paths (relative to `remote.php/dav/files/{username}/`) under [remoteFolder],
    ...
    private fun propfindDepthOne(folder: String): List<DavEntry> {
        val url = filesDavUrl(username, folder)
        val request = Request.Builder()
            .url(url)
            .header("Authorization", authHeader)
            .method("PROPFIND", propfindBody)
            ...
            .build()
        http.newCall(request).execute().use { response ->
            if (!response.isSuccessful) error("PROPFIND failed: HTTP ${response.code}")
    ...
    private fun folderDavHref(folder: String): String {
        ...
        return "/remote.php/dav/files/$username/$suffix"
    }
